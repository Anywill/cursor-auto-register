...
def setup_driver(translator=None):
    ...
    try:
        ...
        # 设置代理（从 config 读取）
        proxy = config.get('Browser', 'proxy', fallback=None)
        if proxy:
            print(f"proxy: {proxy}")
        ...
        try:
            ...
            if os.path.exists(extension_path):
                ...
                print(f"{Fore.GREEN}✅ {translator.get('register.extension_loaded')}{Style.RESET_ALL}")
            else:
                print(f"{Fore.RED}❌ {translator.get('register.extension_load_error', error='Extension path not found')}{Style.RESET_ALL}")
        except Exception as e:
            if translator:
